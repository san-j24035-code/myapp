...
const requestRecipe = async (ingredients, meal, minutes) => {
  const response = await fetch('/api/recipe', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ingredients, meal, minutes }) });
...
function App() {
  ...
  const createSuggestion = async () => {
    ...
    let recipe;
    try { recipe = await requestRecipe(ingredients, meal, minutes); } catch { recipe = buildIngredientRecipe(ingredients, minutes); }
